...
namespace dal
{
    ...
    static void SetParameterValue(MYSQL_BIND* param, enum_field_types type, const void* value, uint32 len,
                                  bool isUnsigned)
    {
    ...
    int SqlPrepare::execute(DBResult* resultSet)
    {
        ...
        if (result)
        {
            ...
            if (resultSet)
            {
                ...
                static_cast<SqlResultSet*>(resultSet)->setResult(m_stmt, result, mysql_stmt_num_rows(m_stmt),
                                                                 mysql_stmt_field_count(m_stmt));
            }
        }
